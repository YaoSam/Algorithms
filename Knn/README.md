...
- 最简单的做法是每次都遍历前ｋ个元素得到最大值。显然不可取。   
 - 复杂度O(n*k)  
- 第二种做法是对前ｋ个元素排序(从大到小)，这样第一个元素就是最大值。每次也只要将比较元素插入到对应位置即可。可以理解为简化版的插入排序。这里虽然加快了最大值的查找。增加的花销是维护前K个元素的序。其实对KNN来说也是没必要的。  
 - 复杂度O(n)~O(n*k)  
-  第三种做法是先在前k元素建立最大堆。首个元素也是最大值。      
